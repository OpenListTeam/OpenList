...
func GetTaskDataByType(type_s string) (*model.TaskItem, error) {
	...
	if err := rwDb.R().Where(task).First(&task).Error; err != nil {
...
func UpdateTaskData(t *model.TaskItem) error {
	return errors.WithStack(rwDb.W().Model(&model.TaskItem{}).Where("key = ?", t.Key).Update("persist_data", t.PersistData).Error)
}

func CreateTaskData(t *model.TaskItem) error {
	return errors.WithStack(rwDb.W().Create(t).Error)
}
