...
func TestQbittorrentAddFromTorrentUploadsTorrentFile(t *testing.T) {
	...
	client, err := qbittorrent.New(server.URL)
	if err != nil {
...
func TestQbittorrentAddFromLinkUsesUrlsFieldAndAcceptsHTTP200(t *testing.T) {
	addCalled := false
...
func TestQbittorrentAddFromLinkReportsNon2xxResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/v2/app/version":
			w.WriteHeader(http.StatusOK)
